...
def type_jduge(mat, i, j):
    min2 = find_min2_index(mat, i, j)
    min1 = find_min_index(mat, i, j)
    min2.sort()
...
def is_edge_point(mat, i, j):
    #针对中心像素点处于斜边缘，递减/递增的对角线上
    min2 = find_min2_index(mat, i, j)
    min2.sort()
    if operator.eq(min2, [0, 1]):
        ls = [mat[i + 2][j - 2], mat[i + 1][j - 1], mat[i - 1][j + 1], mat[i - 2][j + 2]]
        if ls[1] < ls[2]:
            if ls[0] < ls[1] and ls[2] < ls[3]:
                return (ls[1] + ls[2]) / 2
            else:
                return 0
        else:
...
padded_noise_mat = np.pad(noise_img, ((5,5), (5,5)), mode='symmetric')
padded_noise_mat = padded_noise_mat.astype(np.float)
denoise_img = padded_noise_mat.copy()                     #XX为去噪图像
            ...
            if padded_noise_mat[i][j] == 0 or padded_noise_mat[i][j] == 255:
                ...
                if noise_point_count(padded_noise_mat,i,j) == 4:       #4邻域点均为信号点
                    ...
                    if num == 0:             #0:4不用判断和A、D哪个更接近
                        ...
                        if is_edge_point(padded_noise_mat,i,j) != 0:   #如果是斜对角线边缘上的点，则用对角线2点均值来代替去噪像素值
                            denoise_img[i, j] = is_edge_point(padded_noise_mat,i,j) #符合条件返回均值，否则返回0
                        else:
                            ...
                            if compare_value(mat_a,ls_a) < compare_value(mat_d, ls_d):   #选择以该估计值作为4邻域点，对所在中心点估计值与中心值较接近的估计值
                                ...
                                denoise_img[i, j] = temp_d
                elif noise_point_count(padded_noise_mat,i,j) == 3: #有1个噪声点，需要用该噪声点所在3*3格子中的信号点临时替换该噪声点，再估计中心点
                    temp_mat = denoise_mat1(padded_noise_mat,i,j)
                    ...
                    if num == 0:             #0:4不用判断和A、D哪个更接近
                        ...
                        continue
                    # xx[i, j] = np.mean([temp_mat[1][0], temp_mat[0][1], temp_mat[1][2], temp_mat[2][1]])
                    else:                      #2:2 比较
                        if is_edge_point(padded_noise_mat,i,j) != 0:   #如果是斜对角线边缘上的点，则用对角线2点均值来代替去噪像素值
                            denoise_img[i, j] = is_edge_point(padded_noise_mat,i,j) #符合条件返回均值，否则返回0
                        else:
                    ...
                    temp_mat = denoise_mat2(padded_noise_mat,i,j)
                    loc = point_loc_type(temp_mat,1,1)
                    num = point_num_type1(temp_mat,1,1)
                    index_d = (loc * 7 + num * 2)
                    index_a = (loc * 7 + num * 2) - 1
                    ...
                    # xx[i, j] = np.mean([temp_mat[1][0], temp_mat[0][1], temp_mat[1][2], temp_mat[2][1]])
                        ...
                        if is_edge_point(padded_noise_mat,i,j) != 0:   #如果是斜对角线边缘上的点，则用对角线2点均值来代替去噪像素值
                            ...
                            if compare_value(mat_a,ls_a) < compare_value(mat_d, ls_d):   #选择以该估计值作为4邻域点，对所在中心点估计值与中心值较接近的估计值
                                ...
                                denoise_img[i, j] = temp_d
                #elif noise_point_count(padded_noise_mat, i, j) == 1 or noise_point_count(padded_noise_mat, i, j) == 0:
                #    xx[i][j] = meanfilter(padded_noise_mat, i, j)
...
mat = img.astype(np.int)
lena_diff = cv2.absdiff(mat,XX)
large_10 = 0 # 比10大的点有1431个, 孤立噪声点为875个,邻域中有1个噪声点的为456个
tt = np.pad(lena_diff, ((2,2), (2,2)), mode='symmetric')
h, w = tt.shape
padded_noise_mat = np.pad(noise_img, ((2,2), (2,2)), mode='symmetric')
for i in range(2, h-2):
    for j in range(2, w-2):
        if tt[i][j] >= 10:
            if noise_point_count(padded_noise_mat, i, j) == 4:
                large_10 += 1
print(large_10)
...
workbook = xlsxwriter.Workbook('C:/Users/AMDyes/Desktop/excel/lena.xlsx') # 建立文件
...
for i in range(0, img.shape[0]):
    for j in range(0, img.shape[1]):
        if noise_img[i][j] == 0 or noise_img[i][j] == 255:
            worksheet2.write(i,j, noise_img[i][j], cell_format1)
        else:
            worksheet2.write(i,j, noise_img[i][j])
        if abs(img[i][j] - XX[i][j]) >= 10:
            worksheet1.write(i, j, img[i][j], cell_format1)
            worksheet3.write(i, j, XX[i][j], cell_format1)
            worksheet4.write(i, j, abs(img[i][j]-XX[i][j]), cell_format1)
        else:
            worksheet1.write(i, j, img[i][j])
            worksheet3.write(i, j, XX[i][j])
            worksheet4.write(i, j, abs(img[i][j] - XX[i][j]))
workbook.close()
