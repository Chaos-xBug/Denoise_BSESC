...
for i in range(noise_img.shape[0]):
    for j in range(noise_img.shape[1]):
        if noise_img[i][j] == 0 or noise_img[i][j] == 255:
            indic_mtx[i][j] == 1
'''
img = cv2.imread('C:/Users/AMDyes/Desktop/result/lena512.bmp', cv2.IMREAD_GRAYSCALE)            #读取不含噪声灰度图
noise_img = cv2.imread('C:/Users/AMDyes/Desktop/noise_img/0.1noise.bmp', cv2.IMREAD_GRAYSCALE)  #读取含噪图，此处固定噪声方便测试效果
padded_img = np.pad(img, ((1,1), (1,1)), mode='symmetric')                                      #边缘填充
...
padded_noise_mat = np.pad(noise_img, ((3,3), (3,3)), mode='symmetric')
